...
-- Run this in Supabase SQL Editor to fix all dashboard errors and type mismatches.

-- 1. Create User Stats v3 (FIXED for uuid vs text)
CREATE OR REPLACE FUNCTION public.get_global_user_stats_v3()
...
CREATE OR REPLACE FUNCTION public.get_global_search_stats_v3()
RETURNS JSONB SECURITY DEFINER SET search_path = public LANGUAGE plpgsql AS $$
DECLARE
    total_count BIGINT;
    top_terms JSONB;
...
CREATE OR REPLACE FUNCTION public.get_global_ai_stats_v3()
RETURNS JSONB SECURITY DEFINER SET search_path = public LANGUAGE plpgsql AS $$
DECLARE
    total_count BIGINT;
    top_questions JSONB;
...
-- 4. Create Sermon Counts v3 (FIXED for uuid)
CREATE OR REPLACE FUNCTION public.get_global_sermon_counts_v3()
...
GRANT EXECUTE ON FUNCTION public.get_global_search_stats_v3() TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_global_ai_stats_v3() TO anon, authenticated, service_role;
